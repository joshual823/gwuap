...
3. Restyle to the mobile-first hybrid design ✅
4. Deploy it live (Vercel) ✅ — LIVE at https://gwuap.vercel.app
5. Dollar amounts, odds/stake dropdowns, unverified badge ← up next
...
## Session 4 — Deploy it live ✅ DONE

**Goal by end of session:** a real public URL anyone can visit.

- [x] Patched Next.js to 14.2.35 (last patched version for the 14.x line —
      Next.js stopped backporting to 13.x/14.x as of May 2026; a future
      upgrade to 15.x/16.x is still on the list before wider public launch)
- [x] Pushed the project to GitHub (github.com/joshual823/gwuap)
- [x] Created a Vercel account, imported the repo, deployed
- [x] Fixed a production crash: environment variables were set as
      "Sensitive" in Vercel, which conflicts with how NEXT_PUBLIC_ variables
      need to work (they must be readable at build time). Switched to
      Claude Code partway through this session to fix it directly via the
      Vercel CLI after repeated dashboard UI issues — it correctly diagnosed
      and fixed the variable visibility, verified the fix in the actual
      deployed bundle, and also scoped the vars to Development so
      `vercel env pull` works for local dev going forward
- [x] Fixed a missing root route (`/` 404'd because there was no
      `app/page.tsx`) — added a redirect from `/` to `/feed`
- [x] Cleaned up a stray empty directory left over from an earlier shell
      command typo
- [x] Confirmed live: `gwuap.vercel.app` redirects to `/feed`, and
      `/feed`, `/login`, `/signup`, `/leaderboard` all load correctly
- [ ] Still to revisit later: re-enable "Confirm email" in Supabase with a
      real email provider before inviting a wider public audience — it's
      currently OFF for testing convenience

**Status: LIVE at https://gwuap.vercel.app**
